...
## Introductie
Voor de beweging richting gegevenslandschap is er behoefte aan API’s voor het bevragen van gegevens bij de bron. Dit koppelvlak beschrijft een API voor het zoeken en raadplegen van kadastrale onroerende zaken.
...
## Scope
In de ontwikkeling van deze koppelvlakstandaard wordt (minimaal) de zoek- en opvraagfunctionaliteit ingevuld zoals die is beschreven in de koppelvlakstandaard RSGB-bevragingen versie 1.0, module kadastrale onroerende zaken. Dit betekent dat minimaal de (combinaties van) zoekparameters die zijn beschreven in services van deze module ook mogelijk zijn met de API "BRK-bevragingen". De definities van de antwoorden kunnen wel wijzigen.
...
## Uitgangspunten
- Startpunt voor informatiemodellen van gegevens in de berichten is RSGB 3.00.02.
...
## Realisatie
Per zoekpad (operatie) uit RSGB-bevragingen versie 1, module kadastrale onroerende zaken wordt een issue (user story) gemaakt. Deze issues worden achtereenvolgens gerealiseerd. Mogelijk kunnen meerdere zoekpaden worden opgelost in dezelfde API operaties.
